...
\section{Related Work} 
    ...
    \subsection{Constraint-based Program Analaysis}
    Types systems can be understood as logic systems, we know this from the dawn of programming languages. Numerous Attempts are made to model type systems using formal logic. Notebaly, constraint logic programming provide an intuitive and declarative platfrom to understand types as logic constraints. In this, type check a program is to find possible solutions of type assignments for each expressions in the source code. Every expression introduce new resitrictions of what the solution can be. A well typed program are feasible and ill-typed program has no solution can be found. Many have tried implement type system using this equivalence. The technique of analyzing a set of constraints often relying on finding some critical subsets:
 ...
 Constraint based type eystem Can be traced back to [The essence of ML type inference], providing an example of generating constraints from ML program, and an implementation of solver that solves these constraints.  [Strategies for solving constraints in program analysis] introduced a set of combinator that are support efficiently order the constraints to improve the efficient and complexity of constraint solver.  SHErrLoc \cite{Zhang2015-xy} uses constraints as the underlying to perform type inference and type error diagnoses. SHErrLoc uses graph analysis technique to determine type error and identify potential causes.  These tools often uses constraints as a simplified model of specifying type system. They often do not offer more benefits from traditional algorimithtic methods [Hindley Milner] other than the simplicity of implementation. In this SHErrLoc provides deeper analysis of the type error, in the form of multiple suggestions and ranking. However, it also involve non-trivial analysis process of the constraints that are hard to reason about its correctness and performance.

 \paragraph{Constraint-based Type Systems Using MUS}

Type error slicing \cite{Haack2004-fr} shows a technique to identify all necessary locations of a type error that is necessary for programmers to diagnose the root cause.  The system described by Haack and Wells uses the concept of Miinimal Unsatisfiable Subset, although a different name (Minimal Unsolvable Constraint Sets) was used in the paper, to find minimal set of location that contribute to the type error. Chameleon \cite{Stuckey2003-pz} uses type error slicing as the underlying technique and a generalized constraint semantics, constraint handling rules, to support type classes and functional dependent types. [Finding Minimum Type Error Sources] is another tool uses a generalized constraint semantics (MaxSMT) and a state-of-art tool QuickExplain algorithm \cite{Junker2004-ds} to find minimal . The study uses a benchmarked the quality  and performance using 345 programs generated by the authors, all programs are fewer than 100 lines. 

\subsection{Finding all possible causes a type error}

SHErrLoc \cite{Zhang2015-xy} is able to find multiple potential causes of type error. 
Counter-factual typing (CFT) \cite{Chen2014-dz,Chen2020-ad} uses a variation-based type system; it is capable of suggesting the correct type for all possible. CFT is able to suggest multiple-location changes, and CFT uses a ranking heuristics to pvoide a predition of the likely hood of each cause. Goanna is different to this tool by providing a more generic solutions. UNlike SHErrLoc needs to use specific language (\todo{name of the language}) and CFT needs to define a bespoke type systems and algorithms, Goanna uses generic constraint format (Prolog program) and analysis approach (MUS, and MCS) to produce the same outcome. And because of this, Goanna is able to benefit from the flexibility and efficiency of these existing systems.


\subsection{Interactive Type Error Debugging}   
ChameleonIDE \cite{Fu2023-fm} improved traditional type error slicing by allowing programmers to more efficiently understand type error by interactively exploring MUS. 
