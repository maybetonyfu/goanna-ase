...
\usepackage{listings}
\usepackage{courier}  % For Courier font (optional)
% Or use any other monospaced font package

\lstset{
  basicstyle=\ttfamily\small,  % Use \small or \footnotesize for size
}%
...
\title{Goanna: A Reasonable Approach for Type Error Debugging}
...
\section{Related Work} 
    ...
    In this section, we survey the corpus of work that Goanna built upon and was inspired by. We first review the work that is built on a theoretical foundation similar to that of Goanna. We then examine studies that share similar aims and offer a debugging experience comparable to Goanna's. Finally, we review research on program slicing and discuss how Goanna extends the concept of slicing and distinguishes itself from its predecessors.

    \subsection{Constraint-based Type Checking}
    Types systems can be understood as logic systems, we know this from the dawn of programming languages \cite{Reis_undated-lt}. Numerous Attempts are made to model type systems using formal logic. Particularly, constraint logic programming provide an intuitive and declarative platfrom to understand types as logic constraints. In this, type check a program is to find possible solutions of type assignments for each expressions in the source code. Every expression introduce new resitrictions of what the solution can be. A well typed program are feasible and ill-typed program has no solution can be found. Many have tried implement type system using this equivalence. Quite commonly, the technique of analyzing a set of constraints often relying on finding some critical subsets:

    – A minimal unsatisfiable subset (MUS) $M$ of a set of constraints $C$ is a subset $M \subseteq C$ such that $M$ is unsatisfiable and $ \forall{c} \in M : M \setminus \{c\}$ is satisfiable. An MUS can be seen as a minimal explanation of the constraint system’s infeasibility. MUSes have been used extensively, mostly in combination with programming slicing, as a means to explain type errors. An MUS of type system constraints reasoning chain connecting all evidence from one location of the conflict to another.
    
    – A minimal correction set (MCS) $M$ of a set of constraints $C$ is a subset $M \subseteq C$ such that $C \setminus M$ is satisfiable and $\forall{S} \subset M : C \setminus S$ is unsatisfiable. MCSes are so named due to the fact that their removal from $C$ can be seen to “correct” the infeasibility. In an ill-typed program, an MCS $C$ can be seen as the ``cause" of a type error; the removal of C will result in the system being well-typed. Goanna uses MCS to represent potential causes of a type error. Each MCS contains the set of locations that need to be changed to fully resolve the type error.

    – A maximal satisfiable subset (MSS) $M$ of a set of constraints $C$ is a subset $M \subseteq C$ such that M is satisfiable and $\forall{c}\ in\ C \setminus M:M\cup\{c\}$ is unsatisfiable. The definition of an MSS is symmetric to that of a MUS, with “satisfiable” and “unsatisfiable” swapped along with maximal for minimal. MCS and MSS are the complement sets of one another. In an ill-typed program, an MSS $M$ can be seen as the resulting typing environment if a type error is fixed by excluding the MCS $C - M$. Goanna uses an MSS to provide type hints for the program even when it is ill-typed.
    
    Not all Constraint-based Type Systems uses these subsets to analyzing type errors. We list some that do not and some that do.

    \paragraph{Constraint-based Type Systems Using Custom Analysis}
    Constraint based type eystem Can be traced back to Pottier and Rémy's type system for ML \cite{PottierUnknown-je}. This work provides an theoretical foundation  of generating constraints for ML program. It defines a customized constraint language and provided the semantics for the solver. Similarly, Hage and Heeren \cite{Hage2006-gc} introduced another constraint-based type system. In addition, this work includes a set of combinators that are support efficiently order the constraints to improve the efficient of the solving and reduce  complexity of constraint solver. These systems often uses constraints as an approach to  simplified the process of specifying type system. They often do not offer more benefits from traditional algorimithtic methods \cite{Damas1982-zw} other than the simplicity of implementation. These systems focus on the providing the theoretical foundation of constraint-based type system rather than concrete implementations.

    
 \paragraph{Constraint-based Type Systems Using MUS}

Type error slicing \cite{Haack2004-fr} shows a technique to identify all necessary locations of a type error that is necessary for programmers to diagnose the root cause.  The system described in this work uses the concept of Miinimal Unsatisfiable Subset, although a different name (Minimal Unsolvable Constraint Sets) was used in the paper, to find minimal set of location that contribute to the type error. Chameleon \cite{Stuckey2003-pz} uses type error slicing as the underlying technique and a generalized constraint semantics, constraint handling rules \cite{Fruehwirth2017-ro}, to support type classes and functional dependent types. [Finding Minimum Type Error Sources] is another tool uses a generalized constraint semantics (MaxSMT) and a state-of-art tool QuickExplain algorithm \cite{Junker2004-ds} to find MUSes. This work is evalueted with a  benchmarked study, on the quality and performance using 345 synthethized programs, all programs are fewer than 100 lines. 

\subsection{Finding all possible causes a type error}
One key feature of Goanna is its ability to provide alternative possible causes. However, there are other approaches to achieve similar results.
SHErrLoc \cite{Zhang2015-xy} is able to find multiple potential causes of type error. SHErrLoc uses a custom defined constraint language (SCL) as type system specification and it performs type inference and type error diagnoses using graph analysis technique. SHErrLoc is able to determine type error and identify multiple potential causes. The authors acknowledged the performance of SHErrLoc remains suboptimal. And SHErrLoc was not tested on relatively small Haskell programs.  Goanna is similar with SHErrLoc in that it uses constraint-based type inference. Goanna is different in that it uses more standardized constraint language (Prolog) and  analysis appraoches (MUSes and MCSes), which Goanna can benefit from that these techniques are well-studied for their application and performance. Goanna share SHErrLoc's ambition of providing a generaic constraint-based analysis platform that support multiple language.

Counter-factual typing (CFT) \cite{Chen2014-dz,Chen2020-ad} uses a variation-based type system based on choice calculus \cite{Erwig2011-cc}; it is capable of suggesting the correct type for all possible. CFT is able to suggest multiple-location changes, and CFT uses a ranking heuristics to pvoide a predition of the likely hood of each cause. The work include an performance analysis of CFT, where it shows that running time can get exponentially long when the choice nesting levels are unbounded.  Goanna provide similar features with CFT, despite using different appraoch. However Goanna and CFT faces simiar the perfomance limitation. It is possible that this limitation comes from the fact the numeber of potential fixes of type errors can often be exponentially when multiple tyep error are fused. The efficient CFT \cite{Chen2020-ad} offer a brute force perfomance optimization, however the optimization is based on the premise that program changes incrementally, and the later type check and benefit from previous computation results. Goanna offers a divide and conquer approach, where we break down the type errors into atomic categories, and fomulate potential paths to optimize for each category.


\subsection{Interactive Type Error Debugging}   
ChameleonIDE \cite{Fu2023-fm} improved traditional type error slicingf in Chameleon \cite{Stuckey2003-pz} by allowing programmers to more efficiently understand type error by interactively exploring the chain of logical reasoning in MUS one step at a time. Hazel \cite{Zhao2024-qd} and Hazel Tutor\cite{Potter2020-tv} are interactive debugging systems that uses innovative ideas like structural editing and type-hole driven development. Goanna provide a similar feature these systems: ill-typed programs can be assigned types so that programmers can be more informed to resolve the errors. Comparing to these tools, GoannaIDE introduce some novel ideas such as selectable potential causes, and type error grouping.
...
For a simplified Haskell syntax shown in Fig.~\ref{fig:translation}.A, we generate a list of Prolog predicates in the language shown in Fig.~\ref{fig:translation}.B. We use 4 auxiliary functions during the constraint translation process (Fig.~\ref{fig:translation}.C) to generate Prolog variables for future unification. \texttt{fresh} makes a unique unbound Prolog variable. \texttt{var} takes a Haskell identifier name and returns a Prolog variable. Naively, this can be done by turning it to uppercase. \texttt{atom} takes a Haskell type constant/constructor name and returns a Prolog atom. Naively, this can be achieved by turning it into lowercase. \texttt{append\_clause} takes a Prolog predicate \textcolor{red}{P} and stores it into the set of all generated Prolog predicates $\mathcal{P}$. To clarify, all \textcolor{blue}{parsed Haskell syntax} are in blue. All \textcolor{red}{generated Prolog syntax} are in red. 
  ...
  \begin{figure}[htb]
    ...
    After the constraint generation phase, Goanna obtains a set of constraints $C$ derived from the source code and is able to query the feasibility of the constraint $C$ or any subsets of $C$ by calling the \texttt{solve} function. Many studied the techniques of finding all MUSes \cite{de-la-Banda2003-jm,Bacchus2015-of, Liffiton2016-xi,Bendik2020-pz}.  Goanna uses the MARCO algorithm \cite{Liffiton2016-xi}, an algorithm that is shown to effeciently enumerate all MUSes, MCSes, and MSSes .
    % In addition to MUSes, we also obtain two other important subsets: Minimal Correction Subsets (MCSes) and Maximal Satisfiable Subsets (MSS). When we use the word subset without specifying the corresponding superset, it should be inferred as the subset of the constraint system $C$. We list these subsets obtained from MUS enumeration and give their type-theoretic interpretation. 
...
\itemize{
  \item{\textbf{Code Generation with Validation}: LLMs could generate initial source code based on user-provided prompts. Theory-based tools, such as Goanna, then check for errors. Detected errors could be fed back to the LLMs for corrections, enhancing the efficiency and accuracy of code development.}
