...
\title{Goanna: The Most Reasonable Type Error Debugging System}
...
In this section we  demonstrate Goanna's features using some real world examples. To showcase the debugging features,  we developed Goanna-IDE, an interactive development environment for Haskell that specialize in type error debugging. It make extensive use of Goanna's type error diagnostics and deliver it to programmers through visualization and interactivity. An online demo of Goanna-IDE is available for evaluation at \cite{Fu2023-bo}. Goanna-IDE includes a file explorer, a text editor, and a debugging panel. Goanna-IDE provides the following key features when type errors are encountered:
    ...
    Goanna comprises 3 phases: constraint generation, MUS enumeration, and post-analysis. In the constraint generation phase, Goanna walks the abstract syntax tree and collects constraints. In the MUS enumeration phase, Goanna enumerates through all MUSes. Lastly, in the post-analysis phase, Goanna applies multiple optimization techniques to reduce the number of MCSes, group MCSes by common properties, and sort them based on heuristics.
...
Using the knowledge of Minimal Unsatisfiable Subsets (MUSes), Minimal Correction Subsets (MCSes), and Maximal Satisfiable Subsets (MSSes), we have developed a method to categorize type errors. This formalization addresses the challenges associated with reasoning about type errors and providing intuitive explanations for them. Using this method, we have identified four primary categories of type errors. More complex type errors observed in practice can be decomposed into one or more basic type errors from these four categories.
...
We compiled two datasets of defective Haskell programs (N=153) for running the evaluation. The first dataset (N=86), named \texttt{Haskell Community}, was sourced from Haskell online discussion forums, each containing one or more type errors. These include StackOverflow, Reddit, and Discord,  top communications channels for Haskell users based on a research survey \cite{Fausak2022-zf}. We focused on discussions where authors encountered type errors and sought help, including only those with solutions accepted by the original authors. The second dataset (N=67), called \textit{Haskell Academic}, was compiled from peer-reviewed research papers (Table. \ref{sec:appendix-a}) that are related to Haskell type errors.
