...
Type systems can be viewed as logical systems, a perspective established since the early days of programming languages \cite{Reis_undated-lt}. Many attempts have been made to model type systems using formal logic. Notable examples include the work of Pottier and Rémy, in which a type system for the ML Language \cite{PottierUnknown-je} is specified in terms of constraints. Succinctly defining a specialized constraint language and solver, their work demostrate the simplicity and flexibility of using constraints as the design tool for programming languages. Similarly, Hage and Heeren \cite{Hage2006-gc} proposed another constraint-based type system with more advanced type level features. Their work also includes a set of combinators that efficiently order constraints, reducing the workload and complexity of the constraint solver. These systems use constraints to simplify the work of constructing type systems. However, apart from simplicity in design and implementation, the use of constraints in these work do not provide significant advantages traditional algorithmic methods \cite{Damas1982-zw}.

Constraint logic, a branch of formal logic system, offers an intuitive and declarative framework for understanding types as logic constraints. In this, type checking a program can be viewed as finding solutions for possible type assignments for each expression in the source code. Each expression in the program introduces new restrictions on potential solutions. A well-typed program represents a set of constraints from which at least one solution can be found, while from an ill-typed program no valid solution exist. Many researchers have sought to implement type systems based on constraint logic, often employing techniques of finding and analyzing a few critical subsets:
